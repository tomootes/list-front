<template>
  <div>
    ...
    <div class="content">
      ...
      <h1 class="content__title" :class="{ 'content__title--turquoise': song.youtubeID }" v-if="song.title">{{
        song.title
      }} [{{ song.creationDate }}] </h1>
      <iframe class="content__video" :src="`https://youtube.com/embed/${song.youtubeID}`" frameborder="0"
        allow="autoplay; encrypted-media" allowfullscreen v-if="song.youtubeID"></iframe>
      ...
      <div class="player" v-if="song.file">
        <div class="container">
          <audio controls>
            <source :src="`${this.$axios.defaults.baseURL}/${song.file.url}`" type="audio/wav">
